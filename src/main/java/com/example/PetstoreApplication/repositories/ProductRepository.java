...
public interface ProductRepository extends JpaRepository<Product, Long> {
  ...
  @Query(value = "select * from Product where ((lower(title) LIKE %?1%) or " +
          "(lower(title) LIKE '?1%') or (lower(title) LIKE '%?1')) and (price >= ?2 and price <= ?3)", nativeQuery = true)
  ...
  @Query(value = "select * from Product where ((lower(title) LIKE %?1%) or " +
          "(lower(title) LIKE '?1%') or (lower(title) LIKE '%?1')) and (price >= ?2 and price <= ?3) order by price", nativeQuery = true)
  ...
  @Query(value = "select * from Product where ((lower(title) LIKE %?1%) or " +
          "(lower(title) LIKE '?1%') or (lower(title) LIKE '%?1')) and (price >= ?2 and price <= ?3) order by  price desc ", nativeQuery = true)
  List<Product> findByTitleOrderByPriceDest(String title, float ot, float Do);

  // Поиск по наименованию,по категории, фильтрация по диапазону цены,
  // сортировка по возрастанию цены
  @Query(value = "select * from Product where category_id=?4 and ((lower" +
          "(title) LIKE %?1%) or (lower(title) LIKE '?1%') or (lower(title) LIKE '%?1')) and (price >= ?2 and price <= ?3) order by  price", nativeQuery = true)
  ...
  @Query(value = "select * from Product where category_id=?4 and ((lower" +
          "(title) LIKE %?1%) or (lower(title) LIKE '?1%') or (lower(title) LIKE '%?1')) and (price >= ?2 and price <= ?3) order by  price desc ", nativeQuery = true)
